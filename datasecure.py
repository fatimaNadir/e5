...
def decrpyt_text(encrypt_text, key):
    try:
        cipher = Fernet(generate_key(key))
...
if choice == "Home":
    ...
    st.markdown("Develop a Streamlit_based secure data storage and retrieval system where: Users store data with a unique passkey.Users decrypt data by providing the correct passkey.Multiple failed attempts result in a forced reauthorization (login page). The system operates entirely in memory without external databases.")
    ...
    if not st.session_state.authenticated_user:
        ...
        if st.button("Encrypt And Save"):
            if data and passkey:
                ...
                save_data(stored_data)
                st.succsee(" ✅ Data encrypted and save sucessfully!")
...
# == data retieve data section ==
    ...
    if not st.session_state.authenticated_user:
        ...
        if not user_data:
            ...
            st.write("Encryted Data Eteries:")
            ...
            if st.button("Decrypt"):
                result = decrpyt_text(encrypted_input,passkey)
                if result:
